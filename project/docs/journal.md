...
| 19 | Database without Docker | No Docker and little disk space. Briefly considered MariaDB, but kept PostgreSQL (hosting support, earlier decisions). Chose a staged path: in-memory fakes, then PGlite (PostgreSQL inside Node), then a native PostgreSQL install. | [03](decisions/03-test-strategy.md#database-path-for-tests) | [10](decisions/10-api-conventions.md#shared-components-spec-piece-1) |
