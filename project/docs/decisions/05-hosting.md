...
| Web service | `users-admin-api`: Node, root `project`, build `npm ci --omit=dev` (no dev tools, so no `embedded-postgres` download and no dev-only audit warning), start `npm start`, Starter ($7), health check `/docs`, auto-deploy on commit |
| Node | 24.21.0, pinned in `project/.nvmrc`, which both Render and `nvm use` read |
| Settings | `DATABASE_URL` (internal URL + `?sslmode=no-verify`), `JWT_SECRET` (generated by Render), `DEMO_MODE=true`, `MAX_USERS=200`, `TRUST_PROXY=2` (measured; see [10](10-api-conventions.md)) |
