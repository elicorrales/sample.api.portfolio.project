...
Live at **https://users-admin-api-98o8.onrender.com/docs**.

| Piece | Setting |
|---|---|
| Workspace / project | My existing workspace (named after my first game) / new project `portfolio-api`, environment `Production` |
| Region | Ohio, where my games already run; the database must share the API's region for the private network |
| Database | `users-api-db`: PostgreSQL 18, Basic-256mb ($6), **1 GB** storage ($0.30; it can grow but never shrink). The `0.0.0.0/0` rule on the database itself deleted, so **all internet traffic is blocked**; the workspace and environment rules (which also cover my games) untouched |
| Web service | `users-admin-api`: Node, root `project`, build `npm ci --omit=dev` (no dev tools, so no `embedded-postgres` download and no dev-only audit warning), start `npm start`, Starter ($7), health check `/docs`. Auto-deploy on commit at first, **then turned off:** I push often as a backup, so deploys are manual (Render still builds from GitHub, from the commit I choose) |
| Node | 24.21.0, pinned in `project/.nvmrc`, which both Render and `nvm use` read |
| Settings | `DATABASE_URL` (internal URL + `?sslmode=no-verify`), `JWT_SECRET` (generated by Render), `DEMO_MODE=true`, `MAX_USERS=200`, `TRUST_PROXY=2` (measured; see [10](10-api-conventions.md)), `CORS_ORIGINS=http://localhost:5173` (added for the client, [journal row 52](../journal.md); the client's own address joins it when it's deployed) |

**Along the way:**

- Render's GitHub connection could see only my game repo; I added this repo from GitHub's side (Settings → Installations → Render)
- I asked why not the $0 database (deleted after 30 days, no backups) and why not 5 or 10 GB (the form allows 1 GB; 200 users take well under 1 MB)
- **I pasted the database URL, password included, into the AI chat.** Outside access is blocked, so it can't be used from the internet; rotating it is optional, and I chose to keep going
...
| Watch out for | **Build filters**: without them, a client-only commit also redeploys the API, and the other way round (less pressing now that the API's auto-deploy is off) | Free plan meters usage in credits |
