...
2. **Next: the admin web client** in `website/`, a single-page app hosted as a **Render static site** (changed from Netlify, decision 05; build filters matter less now that the API's auto-deploy is off); `CORS_ORIGINS` on Render already allows `http://localhost:5173`; add the client's address when it's deployed
   ...
   4. **Now: the Users screen, test-first** (journal row 51). **Done:** types from the spec (`npm run api:types`), `openapi-fetch`, MSW; the list gets a demo token on open (memory only) and shows page 1; **5 client tests green** (unreachable, `401` token, `500` list, normal case, heading). Tests use `http://fake-api.test` and the client looks up `fetch` per call, so tests can't reach Render (checked by breaking it). **Seen live in the browser** (journal row 52): `npm run dev` shows the real 45 users from Render, unstyled. **Notebook look done** (row 53): `src/styles/notebook.css`, fonts from Fontsource; left page list, right page blank note; only working controls shown. **B, open a user: done** (row 54): right page, display only; shared in-memory token (`src/api/token.ts`), `src/api/call.ts`; **11 client tests green**. **The token on screen: done** (row 55, decision 13): Get demo token button, no-token page with reasons (unreachable, `429` countdown, API's words), corner countdown, expiry and refused pages; **17 client tests green**. **Paging and search: done** (row 56, decision 13): **24 client tests green**. **Security thinking documented** (row 57, **decision 14**: dev-tools attacks checked against API tests; vandalism of shared demo data is the real exposure, 5 ideas not built). **Search box tried by hand** (row 58, decision 14): Render's Cloudflare firewall blocks `'; DROP TABLE users;--` (`403`) and the page wrongly says "Can't reach the API"; everything else as predicted. **Next:** decide the wording for "no readable answer"; zero-width space via `copy("le\u200Be")`; NUL via console `fetch`. Original list (decision 14's table): bad data, too much (maxlength removed in DevTools, or pasted), SQL and LIKE wildcards (`%`, `_`), HTML/script injection shown as text, special Unicode (emoji, accents like José vs jose, right-to-left text, zero-width and combining characters, a NUL character, which PostgreSQL text can't store), newlines and tabs pasted, `&page=999` typed as text; plus recovery (after a `400` or `500`, the box still works) and how errors and warnings show. **Add, edit, delete, restore: done** (row 59, decision 13): one form on the right page, `400` by field, `409`, `412` with Load their version, Include deleted, Restore; **39 client tests green**; tried against the live API. **Experiments tab: done** (row 60): flood first and live (150 at once), 6 replays with stamps; **47 client tests green**. **Published** (row 61): **https://users-admin-web.onrender.com**, static site `users-admin-web` (root `website`, build `npm ci && npm run build`, publish `dist`, auto-deploy off); API `CORS_ORIGINS` = `http://localhost:5173,https://users-admin-web.onrender.com`; README links it. Earlier picks:
