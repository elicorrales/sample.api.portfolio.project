...
2. **Next: the admin web client** in `website/`, a single-page app hosted as a **Render static site** (changed from Netlify, decision 05; build filters matter less now that the API's auto-deploy is off); `CORS_ORIGINS` on Render already allows `http://localhost:5173`; add the client's address when it's deployed
   ...
   4. **Now: the Users screen, test-first** (journal row 51). **Done:** types from the spec (`npm run api:types`), `openapi-fetch`, MSW; the list gets a demo token on open (memory only) and shows page 1; **5 client tests green** (unreachable, `401` token, `500` list, normal case, heading). Tests use `http://fake-api.test` and the client looks up `fetch` per call, so tests can't reach Render (checked by breaking it). **Seen live in the browser** (journal row 52): `npm run dev` shows the real 45 users from Render, unstyled. **Notebook look done** (row 53): `src/styles/notebook.css`, fonts from Fontsource; left page list, right page blank note; only working controls shown. **Next:** paging, search, open a user, save/`412`, delete/restore, create, `400` by field, `429` countdown, token countdown. Earlier picks:
